    ...
    ui(new Ui::Planner)
{
    ...
    ui->tableWidget_planner->setColumnWidth(3, 64);
...
{
    //high speed
    ui->cBox_highReadSpeed_planner->setVisible(ix.Speed_Fun);
    ...
    gainDelegate->setRange(ix.Gain_Min, ix.Gain_Max);
    ...
    for (int x = 1; x < rowOfData.size(); x++)
    {
        ...
        for(int y = 1; y < rowData.size(); y++)
        {
            if(rowData[y] != " ")
            {
                ...
                item1->setData(Qt::DisplayRole,rowData[y].toDouble());
