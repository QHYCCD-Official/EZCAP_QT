    ...
    ui(new Ui::About)
{
    ...
    ui->label_PCIECardVer->setFont(QFont("Tahoma"));
}
...
void About::updatePCIECardVer(QString str)
{
    ui->label_PCIECardVer->setText(str);
}
...
//    {
//        OUTPUT_ERROR("can't support GetQHYCCDPCIECardNum&GetQHYCCDPCIECardVer functions");
//        return;
//    }
...
//    OUTPUT_INFO("card num = %d", num);
//    for(uint32_t i = 0; i < num; i++)
//    {
//        libqhyccd->GetQHYCCDPCIECardVer(i, &year, &month, &day, &subday);
//        strVer += "PCIE Card " + QString::number(i) + " : " + QString::number(year) + "-" + QString::number(month) + "-" + QString::number(day) + "-" + QString::number(subday);
//        OUTPUT_INFO("strVer = %s", qPrintable(strVer));
//        if(i < num - 1) strVer += "\n";
//    }
//    ui->label_PCIECardVer->setText(strVer);
}
