    ...
    ui(new Ui::FitHeader)
{
    ...
    if (btn)
    {
        ...
        opt.text = btn->text();
#if (QT_VERSION >= QT_VERSION_CHECK(6, 0, 0))
        QSize s = (btn->style()->sizeFromContents(QStyle::CT_HeaderSection, &opt, QSize(), btn).expandedTo(QSize(60, 20)));
#else
...
int FitHeader::FITRead_Common(QString filename, uint32_t *ImgW, uint32_t *ImgH, uint32_t *ImgB, uint32_t *ImgC, uint8_t *ImgData)
{
    fitsfile *fptr;
    int status = 0, imgtype, naxis, maxdim = 3;
    long naxes[3] = { 0 };
    long fpixel = 1;
    ...
    /* Get image params (size dimension color/mono) */
    fits_get_img_param(fptr, maxdim, &imgtype, &naxis, naxes, &status);
    OutputDebug("QHYCCDEZCAP | %s | %s | imgtype = %d naxis = %d naxes = %d %d %d", __FILE__, __FUNCTION__, imgtype, naxis, naxes[0], naxes[1], naxes[2]);

    *ImgW = static_cast<uint32_t>(naxes[0]);
    *ImgH = static_cast<uint32_t>(naxes[1]);
    *ImgB = static_cast<uint32_t>(imgtype);
    *ImgC = (naxis==2) ? 1 : 3;

    if(*ImgW!=ix.FrameW || *ImgH!=ix.FrameH || *ImgB!=ix.FrameB || *ImgC!=ix.FrameC)
    {
        return 1;
    }
    ...
    uint8_t *buffer = static_cast<uint8_t *>(malloc(*ImgW * *ImgH * *ImgB * *ImgC / 8));
    fits_read_img(fptr, datatype, fpixel, nelements, nullptr, buffer, nullptr, &status);
    if(*ImgC == 3)
    {
        for(int j = 0; j < *ImgW * *ImgH; j++)
        {
            if(*ImgB == 8)
            {
                ImgData[3*j]   = buffer[j + *ImgW * *ImgH * 2];
                ImgData[3*j+1] = buffer[j + *ImgW * *ImgH];
                ImgData[3*j+2] = buffer[j];
            }
            else
            {
                ImgData[6*j]   = buffer[2*j   + *ImgW * *ImgH * 4];
                ImgData[6*j+1] = buffer[2*j+1 + *ImgW * *ImgH * 4];
                ImgData[6*j+2] = buffer[2*j   + *ImgW * *ImgH * 2];
                ImgData[6*j+3] = buffer[2*j+1 + *ImgW * *ImgH * 2];
                ImgData[6*j+4] = buffer[2*j];
                ImgData[6*j+5] = buffer[2*j+1];
            }
        }
    }
    else
    {
        memcpy(ImgData, buffer, *ImgW * *ImgH * *ImgC * (*ImgB / 8));
    }
...
void FitHeader::FITWrite(QString nameStr, unsigned char *Buf)
{
    ...
    int EffectiveX,EffectiveY,EffectiveW,EffectiveH;
//    IplImage *FitImg,*OnlyImg;
//    FitImg = cvCreateImage(cvSize(ix.FrameW ,ix.FrameH ), IPL_DEPTH_16U, 1 );
//    //FitImg->imageData = (char*)Buf;
//    memcpy(FitImg->imageData, Buf, FitImg->imageSize);
    Mat FitImg, OnlyImg;
    FitImg.create(Size(ix.FrameW, ix.FrameH), CV_16UC1);
//    FitImg.data = Buf;
    memcpy(FitImg.data, Buf, FitImg.cols*FitImg.rows*(FitImg.depth()==CV_8U?1:2)*FitImg.channels());

    if(ix.IgnoreOverscan)//mainMenuBar->actIgnoreOverScanArea->isChecked())
    {
        EffectiveX = ix.EffectiveX;
        EffectiveY = ix.EffectiveY;
        EffectiveW = ix.EffectiveW;
        EffectiveH = ix.EffectiveH;
    }
    else
    {
        EffectiveX = 0;
        EffectiveY = 0;
        EffectiveW = ix.FrameW ;
        EffectiveH = ix.FrameH ;
    }
//    OnlyImg = cvCreateImage(cvSize(EffectiveW,EffectiveH), IPL_DEPTH_16U, 1 );
    OnlyImg.create(Size(EffectiveW, EffectiveH), CV_16UC1);

//    cvSetImageROI(FitImg, cvRect(EffectiveX,EffectiveY,EffectiveW,EffectiveH));
//    cvCopy(FitImg, OnlyImg, NULL);
//    cvResetImageROI(FitImg);
    Mat matRoi = FitImg(Rect(EffectiveX,EffectiveY,EffectiveW,EffectiveH));
    matRoi.copyTo(OnlyImg);
    matRoi.release();
    //----end----
    ...
    long naxes[2] = {EffectiveW, EffectiveH };
    char ss[81];
    char keywords[100];
...
#if (QT_VERSION >= QT_VERSION_CHECK(6, 0, 0))
    QByteArray encodedString = QStringEncoder("GB18030")(nameStr);
#else
    ...
    if(status != 0)
        OutputDebug("EZCAP | %s | %s | status = %d", __FILE__, __FUNCTION__, status);
...
//    fits_write_img(fptr, TUSHORT, fpixel, nelements, &FitImg->imageData[0], &status);
    fits_write_img(fptr, TUSHORT, fpixel, nelements, &OnlyImg.data[0], &status);
    if(status != 0)
        OutputDebug("EZCAP | %s | %s | status = %d", __FILE__, __FUNCTION__, status);
...
//    FitImg->imageData = (char*)Buf;
    ...
    fits_create_file(&fptr, /*str.c_str()*/encodedString.constData(), &status);
    if(status != 0)
        OutputDebug("EZCAP | %s | %s | status = %d", __FILE__, __FUNCTION__, status);
...
//    fits_write_img(fptr, TUSHORT, fpixel, nelements, &FitImg->imageData[0], &status);
    fits_write_img(fptr, TUSHORT, fpixel, nelements, &FitImg.data[0], &status);
    if(status != 0)
        OutputDebug("EZCAP | %s | %s | status = %d", __FILE__, __FUNCTION__, status);
...
void FitHeader::FITwrite_Live(QString filename, int ImageWidth, int ImageHeight, int ImageBpp, int ImageChannel, unsigned char *Buf)
{
    fitsfile *fptr;
    int  status = 0;//, ii, jj;
    char ss[81];
    ...
    long naxis = 0, nelements = 0;//维数, 数据长度
    long naxesColor[3] = {ImageWidth, ImageHeight, ImageChannel};
    long fpixel = 1;

    if(ImageBpp == 8 && ImageChannel == 1)
    {
        bitpix    = BYTE_IMG;
        naxis     = 2;
        nelements = naxesColor[0] * naxesColor[1];
        datatype  = TBYTE;
    }
    else if(ImageBpp == 16 && ImageChannel == 1)
    {
        bitpix    = USHORT_IMG;
        naxis     = 2;
        nelements = naxesColor[0] * naxesColor[1];
    ...
    else if(ImageBpp == 8 && ImageChannel == 3)
    {
        bitpix    = BYTE_IMG;
        naxis     = 3;
        nelements = naxesColor[0] * naxesColor[1] * naxesColor[2];
        datatype  = TBYTE;
    }
    else if(ImageBpp == 16 && ImageChannel == 3)
    {
        bitpix    = USHORT_IMG;
        naxis     = 6;
        nelements = naxesColor[0] * naxesColor[1] * naxesColor[2];
    ...
    fits_create_img(fptr, bitpix, naxis, naxesColor, &status);
    if(status != 0)
        OutputDebug("EZCAP | %s | %s | status = %d", __FILE__, __FUNCTION__, status);
    ...
    unsigned char *bufConvert = (unsigned char *)malloc(ImageWidth*ImageHeight*ImageBpp*ImageChannel/8);
    if(ImageChannel == 3)
        for(int i = 0; i < ImageChannel; i ++)
            for(int j = 0; j < ImageWidth*ImageHeight; j++)
                bufConvert[j+ImageWidth*ImageHeight*i] = Buf[3*j+ImageChannel-i-1];
    else
        memcpy(bufConvert, Buf, ImageWidth*ImageHeight*ImageBpp*ImageChannel/8);

    fits_write_img(fptr, datatype, fpixel, nelements, &bufConvert[0], &status);
    if(status != 0)
        OutputDebug("EZCAP | %s | %s | status = %d", __FILE__, __FUNCTION__, status);
...
void FitHeader::camera_connected()
{
    ...
    for(int i=0; i<rowN; i++)
    {
        ...
        if(iKey == "XPIXSZ")
        {
            ui->tableWidget_fitHeader->setItem(i, 0, new QTableWidgetItem(QString::number(ix.CCD_PixelW)));
        }
        else if(iKey == "YPIXSZ")
        {
            ui->tableWidget_fitHeader->setItem(i, 0, new QTableWidgetItem(QString::number(ix.CCD_PixelH)));
        }
        else if(iKey == "CAMERA")
        {
...
void FitHeader::fitHeaderInfo_changed()
{
    ...
    for(int i=0; i<rowN; i++)
    {
        ...
        {
            ui->tableWidget_fitHeader->setItem(i, 0, new QTableWidgetItem(QString::number(ix.FrameW)));
        }
        else if(iKey.compare("NAXIS2") == 0)
        {
            ui->tableWidget_fitHeader->setItem(i, 0, new QTableWidgetItem(QString::number(ix.FrameH)));
        }
        ...
        {
            if(ix.Cooler_Fun)
                ui->tableWidget_fitHeader->setItem(i, 0, new QTableWidgetItem(QString::number(ix.Temp_Target, 'f', 1)));
        }
        else if(iKey.compare("CCD-TEMP") == 0)
        {
            if(ix.Cooler_Fun)
                ui->tableWidget_fitHeader->setItem(i, 0, new QTableWidgetItem(QString::number(ix.Temp_Now, 'f', 1)));
        }
